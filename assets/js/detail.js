...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderFixedHeader(data) {
        ...
        let serviceHtml = `
            <span class="service-color-dot" style="background-color: ${data.service_color || '#aaa'};"></span>
    ...
    function renderStations(data) {
        ...
        stationIds.forEach((id, index) => {
            ...
            const directly = s.directly || false;
            const color1 = s.color1 || '#888';
            const color2 = s.color2 || color1;
            ...
            // 连接线样式：上半部分颜色取上一站的 color2（若上一站 directly=false 则取 color1），下半部分取当前站的 color1
            let lineHtml = '';
            if (index > 0) {
                const prev = stations[stationIds[index-1]];
                const prevColor2 = prev.directly ? (prev.color2 || prev.color1) : prev.color1;
                const currColor1 = color1;
                lineHtml = `<div class="station-line" style="background: linear-gradient(to bottom, ${prevColor2} 0%, ${prevColor2} 50%, ${currColor1} 50%, ${currColor1} 100%);"></div>`;
            }
            ...
            html += `
                <div class="station-item-vertical">
                    ...
                    <div class="station-right">
                        ...
                        <div class="station-name-secondary">${stationName.secondary}</div>
